...
describe("newsMedia", () => {
  ...
  it("builds playable video embed markup", () => {
    expect(buildNewsVideoEmbedHtml("https://cdn.example.com/demo.mp4")).toBe(
      '<p><video class="news-inline-video" controls preload="none" playsinline src="https://cdn.example.com/demo.mp4"></video></p>'
    );
  });

  it("normalizes existing video embeds to avoid metadata preloading", () => {
    expect(
      normalizeNewsMediaHtml(
        '<p><video class="news-inline-video" controls preload="metadata" src="https://cdn.example.com/demo.mp4"></video></p>'
      )
    ).toContain('preload="none"');
    expect(
      normalizeNewsMediaHtml(
        '<p><video class="news-inline-video" src="https://cdn.example.com/demo.mp4"></video></p>'
      )
    ).toContain('<video preload="none"');
  });
