...
const MEMBER_FILTER_OPTIONS: MemberFilter[] = [
  "all",
  "pending",
  "approved",
  "rejected",
...
export function AdminMembers() {
  ...
  const handleConfirm = () => {
    ...
    switch (type) {
      ...
      case "reopen":
        setMembers(
          members.map((m) =>
            m.id === memberId ? { ...m, status: "pending" as const } : m
          )
        );
        if (selectedMember?.id === memberId) {
          setSelectedMember({ ...selectedMember, status: "pending" });
        }
        break;
      case "delete":
  ...
  const handleExportToExcel = () => {
    ...
    link.setAttribute(
      "download",
      `MACMAA_Members_${new Date().toISOString().split("T")[0]}.csv`
    );
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              {MEMBER_FILTER_OPTIONS.map((status) => (
                <button
                  key={status}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <InfoField
                      ...
                      value={selectedMember.chineseName}
                    />
                    <InfoField
                      label={t("admin.members.detail.englishName")}
                      value={selectedMember.englishName}
                    />
                    <InfoField
                  ...
                  <div className="border-t pt-4 mt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <InfoField
                        ...
                        value={selectedMember.emergencyName}
                      />
                      <InfoField
                        label={t("admin.members.detail.emergencyPhone")}
                        value={selectedMember.emergencyPhone}
                      />
                      <InfoField
                        label={t("admin.members.detail.emergencyRelation")}
                        value={selectedMember.emergencyRelation}
                      />
                      <InfoField
                        label={t("admin.members.table.applyDate")}
                        value={selectedMember.applyDate}
                      />
                    </div>
                  </div>
                </div>
