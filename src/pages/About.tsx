...
import { Heart, Users, Calendar, BookOpen, Award, Target, Sparkles, Check, MapPin, Phone, Mail, Video, Play } from 'lucide-react';
...
export function About() {
  ...
  const achievements = [
    {
      icon: Award,
      title: language === 'zh' ? 'Manningham市府认可' : 'Manningham Council Recognition',
      color: '#2B5F9E',
    },
    {
      icon: Users,
      title: language === 'zh' ? '多元文化部门支持' : 'Multicultural Department Support',
      color: '#6BA868',
    },
    {
      icon: Sparkles,
      title: language === 'zh' ? '中国驻墨尔本总领馆认可' : 'Chinese Consulate Recognition',
      color: '#EB8C3A',
    },
  ];

  const services = [
    language === 'zh' ? '每年举办超过30场大型、中型、小型社区活动' : 'Over 30 community events annually',
    language === 'zh' ? '提供居家养老、健康、政府补助等公益讲座' : 'Public seminars on aged care, health, and government support',
    language === 'zh' ? '组织文化节庆（春节、中秋、端午等）' : 'Cultural festivals (Chinese New Year, Mid-Autumn, Dragon Boat)',
    language === 'zh' ? '举办舞蹈、太极、乒乓球、书法乐等课程' : 'Dance, Tai Chi, table tennis, calligraphy, and music classes',
    language === 'zh' ? '带领社区参加政府与多元文化活动' : 'Leading community participation in government and multicultural events',
    language === 'zh' ? '建立艺术团、文娱小组、长者支持计划' : 'Arts groups, entertainment teams, and senior support programs',
    language === 'zh' ? '为新移民提供信息协助与社区连接' : 'Information assistance and community connections for new migrants',
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      >
        <h1 className="text-[#2B5F9E] mb-4 sm:mb-6 text-3xl sm:text-4xl md:text-5xl lg:text-6xl px-2">{t('nav.about')}</h1>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="mt-8 sm:mt-10 text-center">
                <h3 className="text-[#2B5F9E] mb-4 text-lg sm:text-xl">
                  {language === 'zh' ? '扫码添加会长微信' : 'Scan to Add WeChat'}
                </h3>
